...
export const GSAPEngine = {
    animate(element, params) {
        ...
        const pendingAnimation = import('gsap').then((module) => {
            gsap = module.default || module;
            return createAnimation(element, params);
        }, () => {
            gsapLoadError = new Error('GSAP is not installed. Please install it: npm install gsap');
            throw gsapLoadError;
        });
        ...
        return {
            ...
            onComplete: (onResolve) => pendingAnimation.then(anim => anim.then(onResolve))
        };
        function createAnimation(el, params) {
