...
export const mapTransitionToMotion = (mercuryTransition, callbacks) => ({
...
export const mapTransitionToAnimeJS = (mercuryTransition, callbacks) => ({
    duration: (mercuryTransition?.duration ?? 0.3) * 1000,
    autoplay: mercuryTransition?.autoplay,
    delay: (mercuryTransition?.delay ?? 0) * 1000,
    ease: mercuryTransition?.type === 'spring'
        ? createSpring({
...
export const mapTransitionToGSAP = (mercuryTransition, callbacks) => ({
...
export function mapToGSAPEasing(easing) {
    if (!easing)
        return easing;
