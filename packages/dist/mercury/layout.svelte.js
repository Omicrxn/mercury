...
import { CompositeProjectionAnimator, createSnapshot, InPlaceMetadataManager, ProjectionAnimationHandler, ProjectionAnimator } from '@layout-projection/animation';
import { LayoutAnimationFramer, LayoutProjectionAnimationHandler, SKIP_SIZE } from '@layout-projection/animation/handlers';
 ...
 */
function buildProjectionTreeDownwards(element, layoutId = uuid(), metadataManager) {
    if (nodeMap.has(element))
        throw new Error('Projection Node already exists for the given element');
 ...
 */
function buildTreeRecursive(element, parentProjectionNode, metadataManager) {
 ...
 */
function tryAttachToNearestParent(projectionNode) {
...
export const layout = ({ layoutId, track, animationConfig = { duration: 300, easing: (progress) => progress } }) => {
    ...
    return (element) => {
        ...
        const layoutHandler = new LayoutProjectionAnimationHandler(layoutFramer, metadataManager);
        const handlers = [layoutHandler];
        const animator = new CompositeProjectionAnimator(handlers);
...
const resetAndMeasure = (projectionNode) => {
...
const snapAndAnimate = (projectionNode, animator, animationConfig) => {
    const isRoot = projectionNode.parent() === null;
    if (!isRoot)
        return;
    ...
    projectionNode.traverse((node) => {
        ...
        if (!previous && !current)
            return;
        if (current && previous?.equals(current))
            return;
